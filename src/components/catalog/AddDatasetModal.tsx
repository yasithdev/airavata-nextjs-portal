...
import { useCatalogTags, useCreateDataProduct } from "@/hooks";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const product = {
        ...
        productSize: formData.size ?? 0,
        format: formData.format || undefined,
        dataProductType: DataProductType.FILE,
      };

      const result = await createDataProduct.mutateAsync(product);
      queryClient.invalidateQueries({ queryKey: ["data-products"] });
      queryClient.invalidateQueries({ queryKey: ["data-products", "public"] });
      ...
      onOpenChange(false);
      router.push(`/datasets/${result.productUri}`);
      setFormData({
        name: "",
        description: "",
        type: ResourceType.DATASET,
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end gap-2 pt-4 border-t">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} disabled={createDataProduct.isPending}>
              Cancel
            </Button>
            <Button type="submit" disabled={createDataProduct.isPending}>
              {createDataProduct.isPending ? "Creating..." : "Create Dataset"}
            </Button>
          </div>
        </form>
      </DialogContent>
